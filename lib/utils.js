...
async function getStackqlCommand(core) {
  ...
  core.info(`path: ${process.env.STACKQL_CLI_PATH}`);
  // i want to ls this path

  try {
    const { stdout, stderr } = await execAsync(`ls -la ${process.env.STACKQL_CLI_PATH}`);
    core.info(`Contents of ${process.env.STACKQL_CLI_PATH}:\n${stdout}`);
    if (stderr) {
        core.error(`Errors: ${stderr}`);
    }
} catch (error) {
    core.error(`Failed to list directory: ${error.message}`);
