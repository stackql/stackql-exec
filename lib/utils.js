...
async function execStackQLQuery(core, command, isCommand) {
  ...
  try {
    // Using ShellJS to execute the command synchronously
    let { stdout, stderr, code } = shell.exec(command, { silent: true });

    // Check the exit code explicitly as ShellJS does not throw errors for non-zero exit codes
    if (code !== 0) {
      core.error(`Command failed with exit code ${code}`);
      core.setFailed(`StackQL command failed with error: ${stderr}`);
      return;
    }
