...
async function execStackQLQuery(core, command, isCommand, onFailure) {
  ...
  try {
    // Execute the command synchronously for simplicity in this example
    const output = shell.exec(command, {silent: true});  // silent: true to avoid echoing the command

    // Logging the output
    if (output.stdout) {
      core.info(`STDOUT: ${output.stdout}`);
      if (!isCommand) {
        core.setOutput('stackql-query-results', output.stdout);
      }
    }
    ...
    if (output.stderr) {
      ...
      if (isCommand) {
        core.setOutput('stackql-command-output', output.stderr);
      } else {
        if (onFailure === 'exit') {
          core.setFailed(`StackQL query reported an error: ${output.stderr}`);
        } else {
          core.info(`StackQL query reported an error (continuing): ${output.stderr}`);
          core.setOutput('stackql-query-error', output.stderr);
        }
      }
    }

    if (output.code !== 0) {
      core.error(`Command failed with exit code ${output.code}`);
      core.setFailed('StackQL command execution failed');
    }
